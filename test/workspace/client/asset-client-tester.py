...
def my_on_manual_timing_control(context):
    ...
    pdu_ch1_data = pdu_ch1.get()
    result = True
    count = 0
    while result:
        pdu_ch1_data['data'] = "CLIENT DATA:  " + str(count)
        ...
        pdu_ch2_data = pdu_ch2.read()
        if pdu_ch2_data == None:
            print('ERROR: hako_asset_pdu_read')
        print(f'{hakopy.simulation_time()}: pdu_ch2_data={pdu_ch2_data}')
