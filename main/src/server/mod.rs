...
impl CoreService for HakoCoreService {
    ...
    ) -> Result<Response<NotifySimtimeReply>, Status>
    {
        ...
        if req.is_read_pdu_done {
            hako::api::asset_notify_read_pdu_done(asset_info.name.as_ptr() as *const u8);
        }
        if req.is_write_pdu_done {
            hako::api::asset_notify_write_pdu_done(asset_info.name.as_ptr() as *const u8);
        }
        ...
        let reply = hakoniwa::NotifySimtimeReply {
            ...
            is_pdu_sync_mode: hako::api::asset_is_pdu_sync_mode(asset_info.name.as_ptr() as *const u8),
